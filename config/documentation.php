<?php

return [

    1 => [

        'packages' => [

            'app' => [

                'name' => 'Admin Panel',

                'description' => 'The elegant TALL stack admin panel for Laravel artisans.',

                'path' => resource_path('filament/1.x/docs'),

                'icon' => 'heroicon-o-chart-bar',

                'package' => 'filament/filament',

            ],

        ],

    ],

    2 => [

        'packages' => [

            'app' => [

                'name' => 'Admin Panel',

                'description' => 'The elegant TALL stack admin panel for Laravel artisans.',

                'path' => resource_path('filament/2.x/packages/admin/docs'),

                'icon' => 'heroicon-o-chart-bar',

                'package' => 'filament/filament',

            ],

            'forms' => [

                'name' => 'Forms',

                'description' => 'The elegant TALL stack form builder for Laravel artisans.',

                'path' => resource_path('filament/2.x/packages/forms/docs'),

                'icon' => 'heroicon-o-document-text',

                'package' => 'filament/forms',

            ],

            'tables' => [

                'name' => 'Tables',

                'description' => 'The elegant TALL stack table builder for Laravel artisans.',

                'path' => resource_path('filament/2.x/packages/tables/docs'),

                'icon' => 'heroicon-o-table',

                'package' => 'filament/tables',

            ],

            'notifications' => [

                'name' => 'Notifications',

                'description' => 'Elegant TALL stack notifications for Laravel artisans.',

                'path' => resource_path('filament/2.x/packages/notifications/docs'),

                'icon' => 'heroicon-o-bell',

                'package' => 'filament/notifications',

            ],

            'spatie-laravel-media-library-plugin' => [

                'name' => 'Spatie Media Library Plugin',

                'description' => 'A plugin to add support for spatie/laravel-medialibrary to Filament.',

                'path' => resource_path('filament/2.x/packages/spatie-laravel-media-library-plugin/docs'),

                'package' => 'filament/spatie-laravel-media-library-plugin',

            ],

            'spatie-laravel-settings-plugin' => [

                'name' => 'Spatie Settings Plugin',

                'description' => 'A plugin to add support for spatie/laravel-settings to Filament.',

                'path' => resource_path('filament/2.x/packages/spatie-laravel-settings-plugin/docs'),

                'package' => 'filament/spatie-laravel-settings-plugin',

            ],

            'spatie-laravel-tags-plugin' => [

                'name' => 'Spatie Tags Plugin',

                'description' => 'A plugin to add support for spatie/laravel-tags to Filament.',

                'path' => resource_path('filament/2.x/packages/spatie-laravel-tags-plugin/docs'),

                'package' => 'filament/spatie-laravel-tags-plugin',

            ],

            'spatie-laravel-translatable-plugin' => [

                'name' => 'Spatie Translatable Plugin',

                'description' => 'A plugin to add support for spatie/laravel-translatable to Filament.',

                'path' => resource_path('filament/2.x/packages/spatie-laravel-translatable-plugin/docs'),

                'package' => 'filament/spatie-laravel-translatable-plugin',

            ],
        ],

    ],

    3 => [

        'packages' => [

            'app' => [

                'name' => 'App Framework',

                'description' => 'The elegant TALL stack app framework for Laravel artisans.',

                'path' => resource_path('filament/3.x/packages/app/docs'),

                'icon' => 'heroicon-o-chart-bar',

                'package' => 'filament/filament',

            ],

            'forms' => [

                'name' => 'Forms',

                'description' => 'The elegant TALL stack form builder for Laravel artisans.',

                'path' => resource_path('filament/3.x/packages/forms/docs'),

                'icon' => 'heroicon-o-document-text',

                'package' => 'filament/forms',

            ],

            'tables' => [

                'name' => 'Tables',

                'description' => 'The elegant TALL stack table builder for Laravel artisans.',

                'path' => resource_path('filament/3.x/packages/tables/docs'),

                'icon' => 'heroicon-o-table',

                'package' => 'filament/tables',

            ],

            'notifications' => [

                'name' => 'Notifications',

                'description' => 'Elegant TALL stack notifications for Laravel artisans.',

                'path' => resource_path('filament/3.x/packages/notifications/docs'),

                'icon' => 'heroicon-o-bell',

                'package' => 'filament/notifications',

            ],

            'actions' => [

                'name' => 'Actions',

                'description' => 'Elegant TALL stack modals and slide-overs for Laravel artisans.',

                'path' => resource_path('filament/3.x/packages/actions/docs'),

                'icon' => 'heroicon-o-inbox',

                'package' => 'filament/actions',

            ],

            'widgets' => [

                'name' => 'Widgets',

                'description' => 'Elegant TALL stack dashboard widgets for Laravel artisans.',

                'path' => resource_path('filament/3.x/packages/widgets/docs'),

                'icon' => 'heroicon-o-chart-bar',

                'package' => 'filament/widgets',

            ],

            'spatie-laravel-media-library-plugin' => [

                'name' => 'Spatie Media Library Plugin',

                'description' => 'A plugin to add support for spatie/laravel-medialibrary to Filament.',

                'path' => resource_path('filament/3.x/packages/spatie-laravel-media-library-plugin/docs'),

                'package' => 'filament/spatie-laravel-media-library-plugin',

            ],

            'spatie-laravel-settings-plugin' => [

                'name' => 'Spatie Settings Plugin',

                'description' => 'A plugin to add support for spatie/laravel-settings to Filament.',

                'path' => resource_path('filament/3.x/packages/spatie-laravel-settings-plugin/docs'),

                'package' => 'filament/spatie-laravel-settings-plugin',

            ],

            'spatie-laravel-tags-plugin' => [

                'name' => 'Spatie Tags Plugin',

                'description' => 'A plugin to add support for spatie/laravel-tags to Filament.',

                'path' => resource_path('filament/3.x/packages/spatie-laravel-tags-plugin/docs'),

                'package' => 'filament/spatie-laravel-tags-plugin',

            ],

            'spatie-laravel-translatable-plugin' => [

                'name' => 'Spatie Translatable Plugin',

                'description' => 'A plugin to add support for spatie/laravel-translatable to Filament.',

                'path' => resource_path('filament/3.x/packages/spatie-laravel-translatable-plugin/docs'),

                'package' => 'filament/spatie-laravel-translatable-plugin',

            ],
        ],

    ],

];
