    ...
    Alpine.data('clipboard', () => ({
        ...
        init() {
            this.codeBlocks.forEach((element, key) => {
                ...
                copyToClipboardBtn.innerHTML = this.clipboardIcon;
                copyToClipboardBtn.id = `clipButton-${key}`;

                ['md:block', 'hidden'].forEach((value) => copyToClipboardBtn.classList.add(value));
                ...
                copyToClipboard.on('success', (element) => {
                    copyToClipboardBtn.innerHTML = this.clipboardCopiedIcon;
                    element.clearSelection();
                    setTimeout(() => copyToClipboardBtn.innerHTML = this.clipboardIcon, 1500);
                });
